 ...
 import { timeData } from './data/demoData'
 let _Users = Users
 let _Books = Books
 let _timeData=timeData
 export default {
   ...
   init() {
     ...
     mock.onGet('/echarts/monitor').reply(config => {
       let { name,publishAt } = config.params
       let mockTimeData = _timeData.filter(timeData => {
         if (name && timeData.name.indexOf(name) === -1) return false
         return true
       })
       return new Promise((resolve, reject) => {
         setTimeout(() => {
